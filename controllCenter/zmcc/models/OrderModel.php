...
class OrderModel extends CI_Model {
    ...
    public function getOrderInfo($order_id) {
        ...
        //砍价日志
        if($data['orderinfo']['order_type']=='K'){
            $data['killorderlog'] = $this->getKillOrderLog($order_id);
        }
        return $data;
    }
     ...
     * @return type
     */
    public function getKillOrderLog($order_id) {
        $orderlog = $this->db->select('k.*,u.nickname,u.user_name,uw.wx_headimg')
                ->from('kill_log k')
                ->join('user u','u.user_id=k.user_id','left')
                ->join('user_wechat uw','uw.user_id=k.user_id','left')
                ->where('order_id=' . $order_id)
                ->order_by('k.creatime', 'ASC')
                ->get();
        return $orderlog->result_array();
    }
