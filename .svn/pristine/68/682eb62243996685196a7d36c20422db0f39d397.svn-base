...
abstract class CI_Wechat_basic {
    ...
    protected function setCache($value) {
        ...
        if (!write_file(APPPATH . 'cache/wechat_fun_token', json_encode($arraytoken))) {
            return $value;
        }
        return FALSE;
    }
    ...
    protected function getCache() {
        ...
        if (!file_exists(APPPATH . 'cache/wechat_fun_token'))
            return FALSE;
        $jsontoken = file_get_contents(APPPATH . 'cache/wechat_fun_token');
        if (!empty($jsontoken)) {
            $arraytoken = json_decode($jsontoken,true);
            if ($arraytoken['expires_in'] < time())
                return false;
            return $arraytoken['access_token'];
        }
        return false;
    }
