...
class UserModel extends CI_Model {
    ...
    public function get_wechat_user_info($openid) {
        $query = $this->db->select("user.*,user_wechat.*,supplier.supplier_id,supplier.status as supplier_status,store.store_id,agent.agent_id,agent.agent_status,agent.agent_group_id,user_group.user_group_name")
                ->from("user_wechat")
                ...
                ->join("user_group", "user_group.user_group_id=user.user_group_id", "left")
     ...
     * 设置用户积分
     */
    public function setUseIntergal($user_id, $intergal, $symbol = "+") {
        return $this->db->where("user_id", $user_id)
                        ->set("user_intergal", "user_intergal" . $symbol . $intergal, false)
                        ->update("user");
    }

    public function check_group($user_id, $integral) {
        $is_up = 0;
        $up_level = array();
        ...
        return array('is_up' => $is_up, 'up_level' => $up_level);
    }
...
//        echo $this->db->last_query();exit;
     ...
     * @return type
     */
    public function getUserMppCount($user_id, $time = 0) {
        ...
        $query = $this->db->select("*")
                ...
                ->where("user_id in (select user_id from zm_agent where agent_status=1)")
    ...
    public function getUserFriendsCount($user_id, $time = 0) {
        ...
        $query = $this->db->select("*")
                ->from("user")
                ->where("parent_user_id", $user_id)
                ->where("user_id not in (select user_id from zm_agent where agent_status=1)")
                ->where("createtime>" . $sqtime)
                ->count_all_results();
        return $query;
    }
