...
class KillProductModel extends CI_Model {
    ...
    public function getKillProducts($page = 1, $limit = 10, $where = '') {
        ...
        foreach ($result_data as &$data) {
            $data['kill_number'] = $this->db->select('count(*) as kill_number')
                            ->from('order o')
                            ->join('order_product p', 'p.master_order_id=o.order_id')
                            ->where('o.order_type="K"')
    ...
    //检查用户是否在该活动中下单
    public function checkKillOrder($user_id, $product_id) {
        $num = $this->db->select('o.order_id')
                ->from('order o')
                ->join('order_product op', '(op.master_order_id=o.order_id and op.product_id='.$product_id.')','left')
                ->where('o.master_order_id="0" and o.user_id = ' . $user_id . '  and o.order_type="K" and o.status="0"')
                ->get()
                ->row_array();

        return !empty($num) ? $num['order_id'] : false;
    }
